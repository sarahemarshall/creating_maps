---
title: "Creating Maps in R: Exercises"
subtile: "Auckland R Ladies Meetup Group"
...
tm_shape(World) + tm_polygons("rand")
```
...
We will now add a marker our current location.  
```{r WZmap_setup}
wz <- "WZ building, AUT university"
wz_gc <- geocode(wz)

g <- qmap(location="WZ building, AUT, NZ", maprange = TRUE, zoom=18,
          maptype = "satellite", 
...
Notice that we used the ```geocode``` function. This function returns the longitude and lattitude of a location
...
# Create the map
map <- get_googlemap(wz, markers = df, path = df, scale = 2, zoom=14)
